...
void configure_parser(cli::Parser& parser) {
	parser.set_required<std::string>("p", "path", "Provide the path to the payload file. This file can either be a raw generated payload (msfvenom/cobaltstrike output) or a PE converted to shellcode via tools like pe2shc.exe. Example of option value: \"C:\\Path\\To\\payload.bin\"");

	parser.set_required<std::string>("o", "outfile", "Specify the output file where the generated header file containing the payload will be saved. Example of option value: \"C:\\Path\\To\\payload.h\"");

	parser.set_optional<std::string>("a", "args","", "Provide the required arguments to be passed to the payload. If the payload is mimikatz.exe converted to shellcode, you can provide arguments to it via this option. Example option value (for mimikatz): \"privilege::debug\". NOTE: If this flag is not specified, ARGV of the current process will be cleared after parsing and will be passed to the payload.");
